...
```typescript
import { openDaumPostScreen } from '#vectorx/nuxt/externals/daum/post'
```
...
```
다음은 VxDaumPostPopup 컴포넌트의 전체 소스코드이다. 해당 코드를 참고하여 커스톰한 주소창 팝업을 만들어 보도록 하자.
```html
<template>
  <slot :modelValue="modelValue" :onUpdate:modelValue="value => (modelValue = value)" :loadZipContainer="loadZipContainer">
    <VxPopup v-model="modelValue" :width="800" :height="600" title="우편번호 조회" @shown="() => loadZipContainer()">
...
import VxPopup from '../dialog/u-popup.vue'
import { openDaumPostScreen, DaumPostResult, DaumPostScreenOptions } from '#vectorx/nuxt/externals/daum/post'
...
const props = withDefaults(defineProps<Props>(), {
  ...
  addLatLng: false,
  apiKey: () => {
    return $vx.components.getComponentsOption('VxDaumPostPopup').apiKey
  },
})
        ...
        openDaumPostScreen(zipContianerEl, {
          ...
          onSelected(data) {
            ...
            if (options.props.addLatLng) {
              $vx.global.kakao.maps.load(() => {
                const geocoder = new $vx.global.kakao.maps.services.Geocoder()

                geocoder.addressSearch(data.address, (results, status) => {
                  if (status === $vx.global.kakao.maps.services.Status.OK && results.length > 0) {
...
export default {
  name: 'VxDaumPostPopup',
}
</script>
```

VxDaumPostPopup은 위의 코드와 같이 slot과 useDaumPostPopup 를 통한 확장을 지원한다. 
...
<script lang="ts" setup>
import { VxDaumPostPopup } from '#components'
import { Props } from '#vectorx/nuxt-vuetify/components/daum/u-daum-post-popup.vue'
...
```
이벤트의 경우, props로 선언되지 않았으므로 inheritAttrs 기본 설정에 의해 VxDaumPostPopup에 전달되게 되어 이벤트가 bypass 되도록 처리되는데 이를 방지하려면 inheritAttrs 설정을 false로 세팅해야 한다.
...
```html
<template>
  <VxPopup v-model="modelValue" :width="800" :height="600" title="커스톰 우편번호 조회 (신규 컴포넌트 생성)" @shown="() => loadZipContainer()">
    <div ref="zipContianerEl" style="width: 100%; height: 100%"></div>
  </VxPopup>
</template>
<script lang="ts" setup>
import { Props, Emits, useDaumPostPopup } from '#vectorx/nuxt-vuetify/components/daum/u-daum-post-popup.vue'
...
const props = withDefaults(defineProps<Props>(), {
  ...
  addLatLng: false,
  apiKey: () => {
    return $vx.components.getComponentsOption('CDaumPostPopup').apiKey
  },
})
const emits = defineEmits<Emits>()
