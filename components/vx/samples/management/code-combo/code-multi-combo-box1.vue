<template>
  <v-card>
    ...
    <template #subtitle
      >VxCodeMultiComboBox 컴포넌트를 사용하여 공통 코드를 Multi 선택이 가능한 콤보박스에 바인딩할 수 있다. <br />
    ...
    <template #text>
      <VxCodeMultiComboBox :grpCd="grpCd" v-model="comboValue" v-model:objectValue="comboObjectValue" style="width: 500px" />
      ...
      <div class="ma-2">
        <VxButton icon="mdi-comment-edit" text="콤보 박스 값 변경" @click="changeComboValue"></VxButton>
        <VxButton icon="mdi-comment-edit" text="그룹 코드 변경" @click="() => (grpCd = grpCd === 'SYS_CD' ? 'WORK_DV_CD' : 'SYS_CD')"></VxButton>
      </div>

      <VxMarkdownViewer :content="desc"></VxMarkdownViewer>
    </template>
  </v-card>
</template>
...
import { useVxCodeList } from '#vectorx/nuxt/management/composables'
import { VxButton } from '#vectorx/nuxt-wijmo/components'
...
function changeComboValue() {
  const codes = useVxCodeList(grpCd.value, true)
