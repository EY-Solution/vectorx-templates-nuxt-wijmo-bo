      ...
      <VxButton
        ...
        @click="
          () => {
            ...
            $vx.logger.info(grid.collectionView.itemsEdited)
          }
        "
      />
      <VxButton text="데이터로드" @click="() => loadData()" />
      ...
      <WjFlexGrid :itemsSource="data" allowDragging="Rows" :initialized="grid.initialize" class="mt-2">
        ...
        <WjFlexGridColumn header="System" binding="system" width="*" :cellTemplate="ctx => useVxCodeValue('SYS_CD', ctx.value)">
          <WjFlexGridCellTemplate cellType="Cell" v-slot="cell">
            <VxCodeInputBox v-model="cell.item.system" grpCd="SYS_CD" />
        ...
        <WjFlexGridColumn binding="desc" header="Description" :width="200">
          <WjFlexGridCellTemplate cellType="Cell" v-slot="cell">
            <VxTextBox v-model="cell.item.desc" />
          </WjFlexGridCellTemplate>
        </WjFlexGridColumn>
      </WjFlexGrid>
...
import { ref, onMounted } from '#vectorx/nuxt'
...
import { WjFlexGrid, WjFlexGridColumn, WjFlexGridCellTemplate, WjInputDate, WjInputNumber, VxButton, VxTextBox } from '#vectorx/nuxt-wijmo/components'
import { VxMarkdownViewer } from '#vectorx/nuxt/components'
import { VxCodeInputBox } from '#vectorx/nuxt-wijmo/management/components'
