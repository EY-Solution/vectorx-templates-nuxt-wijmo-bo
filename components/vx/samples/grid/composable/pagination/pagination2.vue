<template>
  <VCard>
    <template #text>
      <VxMarkdownViewer :content="pagination2_1" />
      ...
      <WjFlexGrid class="mt-2" :initialized="grid.initialize" style="height: 300px">
        ...
        <WjFlexGridColumn binding="sysCd" header="시스템" :cellTemplate="ctx => useVxCodeValue('SYS_CD', ctx.value)" :width="100" />
        <WjFlexGridColumn binding="url" header="URL" :width="300" />
        <WjFlexGridColumn binding="httpMethCd" header="Method" :width="100" />
      </WjFlexGrid>

      <VxMarkdownViewer :content="pagination2_2" />
    </template>
  </VCard>
</template>
