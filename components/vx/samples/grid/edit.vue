<template>
  <v-card>
    ...
    <template #text>
      <div>
        <VxButton @click="addRow" text="로우 추가"></VxButton>
        <VxButton @click="removeRow" text="로우 삭제"></VxButton>
      </div>
      <div>
        <VxButton @click="getGridData" text="전체 데이터 조회"></VxButton>
        <VxButton @click="getEditedGridData" text="수정 데이터 조회"></VxButton>
        <VxButton @click="getAddedGridData" text="추가 데이터 조회"></VxButton>
        <VxButton @click="getDeletedGridData" text="삭제 데이터 조회"></VxButton>
      </div>
      <div>
        그리드 by template
        <WjFlexGrid ref="grid" :itemsSource="data" style="max-height: 300px" :allowDelete="true">
          ...
          <WjFlexGridColumn :header="'IP'" :binding="'ip'" :width="200">
            <wj-flex-grid-cell-template cellType="CellEdit" v-slot="cell">
              <VxIpAddressTextBox v-model="cell.value" />
            </wj-flex-grid-cell-template>
          </WjFlexGridColumn>
        </WjFlexGrid>
      </div>
      ...
      <!-- <VxMarkdownViewer>{{ edit1 }}</VxMarkdownViewer> -->
    </template>
  </v-card>
</template>
...
import { WjInputNumber, VxIpAddressTextBox } from '#vectorx/nuxt-wijmo/components'
import { ref } from '#vectorx/nuxt'
import { VxMarkdownViewer } from '#vectorx/nuxt/components'
// import edit1 from './md/edit1.md'
import { useWjGridDataMap } from '#vectorx/nuxt-wijmo/composables'
