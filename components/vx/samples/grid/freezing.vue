<template>
  <v-card>
    ...
    <template #text>
      <div>
        <VxButton @click="() => (grid.control.frozenColumns = 2)" text="컬럼 틀고정"></VxButton>
        <VxButton @click="() => (grid.control.frozenRows = 2)" text="로우 틀고정"></VxButton>
        <VxButton
          @click="
            () => {
      ...
      <VxMarkdownViewer>{{ freezing1 }}</VxMarkdownViewer>
    </template>
  </v-card>
</template>
