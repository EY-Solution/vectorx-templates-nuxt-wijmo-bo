...
import { nextTick, ref } from '#vectorx/nuxt'
...
import { wijmoGrid, wijmo } from '#vectorx/nuxt-wijmo'
...
const grid = useWijmoFlexGrid({
  onAfterInitialized(control) {
    ...
    control.loadedRows.addHandler(() => {
      ...
      // 건수 입력 후, right 정렬
      control.columnFooters.setCellData(0, 0, $vx.utils.formatting.currency(grid.collectionView.sourceCollection.length) + ' 건')
...
const generateData = (cnt: number) => {
  ...
  for (let i = 0; i < cnt; i++) {
    data.push({
      id: i,
      country: countries[$vx.utils.random.max(countries.length) - 1],
