<template>
  <v-card>
    ...
    <template #text>
      <VxButton text="Load Data" @click="grid.pagination.loadPageData()" />
      <VxButton text="Init Data" @click="grid.pagination.currentPageNo.value = 0" />
      ...
      <VxMarkdownViewer>{{ aggregation2 }}</VxMarkdownViewer>
    </template>
  </v-card>
</template>
    ...
    onAfterLoadPageData(pageNo) {
      ...
      if (pageNo > 0) {
        ...
        grid.control.value.columnFooters.setCellData(
          0,
          1,
          $vx.utils.formatting.currency(grid.pagination.collectionView.sourceCollection.length) + ' 건',
        )
