<template>
  <VCard>
    <template #text>
      <h3>upload 이미지 조회</h3>
      <br />
      업로드 된 이미지 파일은 "$vx.management.file.getViewUrl" 메소드를 사용하여 조회할 수 있다. <br />
      ...
      <br /><br />
      "$vx.management.file.getWebAccessUrl" 메소드를 사용하여 파일 정보에서 저장된 Web Server 접근 URL을 조회할 수 있다. <br />
      해당 메소드는 fileId, fileNo, fileGroupId를 인자 값으로 받으며 모든 값은 필수이다.
      <br /><br />
      <VxButton text="Image URL 조회" @click="getWebImageUrl" />
...
import { VxMarkdownViewer, VxWjInputMask, WjInputNumber, VxButton } from '#vectorx/nuxt-wijmo/components'
...
async function getWebImageUrl() {
  webFileImageUrl.value = await $vx.management.file.getWebAccessUrl(fileId.value, fileNo.value, 'vx-sample-image')
}
</script>
