<template>
  <div>
    ...
    <v-card>
      ...
      <template #subtitle> VxDateBox 컴포넌트를 사용하여 일자 입력을 처리할 수 있다. </template>
      <template #text>
        ...
        <VxMarkdownViewer>{{ date1 }}</VxMarkdownViewer>
      </template>
    </v-card>
    <v-card>
      ...
      <template #text>
        <div class="samplebox">
          <VxDateBox v-model="inputValue2" mode="date" />
          <div>
            <v-chip class="ma-2" color="pink" label text-color="white">
              <v-icon start icon="mdi-label"></v-icon>
          ...
          <VxDateBox v-model="inputValue3" mode="datetime" />
          <div>
            <v-chip class="ma-2" color="pink" label text-color="white">
              <v-icon start icon="mdi-label"></v-icon>
          ...
          <VxDateBox v-model="inputValue4" mode="year" />
          <div>
            <v-chip class="ma-2" color="pink" label text-color="white">
              <v-icon start icon="mdi-label"></v-icon>
          ...
          <VxDateBox v-model="inputValue5" mode="yearmonth" />
          <div>
            <v-chip class="ma-2" color="pink" label text-color="white">
              <v-icon start icon="mdi-label"></v-icon>
        ...
        <VxMarkdownViewer>{{ date2 }}</VxMarkdownViewer>
      </template>
    </v-card>
    <v-card>
      ...
      <template #text>
        <div class="samplebox">
          <VxDateBox v-model="inputValue6" mode="datetime" displayFormat="yyyy-MM" valueFormat="yyyyMMddHHmm" />
          <v-chip class="ma-2" color="pink" label text-color="white">
            <v-icon start icon="mdi-label"></v-icon>
            inputValue6 : {{ inputValue6 }}
          </v-chip>
        </div>
        <VxMarkdownViewer>{{ date3 }}</VxMarkdownViewer></template
      >
    </v-card>
    <v-card>
      ...
      <template #text>
        ...
        <VxMarkdownViewer>{{ date4 }}</VxMarkdownViewer>
      </template>
    </v-card>
    <v-card>
      ...
      <template #text>
        <VxDateBox v-model="inputValue8" mode="datetime" :isDefaultValue="true" />
        <v-chip class="ma-2" color="pink" label text-color="white">
          <v-icon start icon="mdi-label"></v-icon>
          inputValue8 : {{ inputValue8 }}
        </v-chip>
        <VxMarkdownViewer>{{ date5 }}</VxMarkdownViewer>
      </template>
    </v-card>
  </div>
</template>
...
import { VxDateBox } from '#vectorx/nuxt-wijmo/components'
