<template>
  <v-card>
    <template #text>
      <h3>Vuetify</h3>
      <br />
      Wijmo 모듈을 참조하지 않을 경우, VxBizNoBox와 VxCorpNoBox는 VTextField 기반의 Vuetify 전용 컴포넌트로 변경된다.

      <br /><br />
      <VxValidationGroup ref="validationGroup">
        <div class="mb-2">사업자 번호</div>
        <VxBizNoBox v-model="value" :required="true" />
        <br />
        <div class="mb-2">법인 번호</div>
        <VxCorpNoBox v-model="value2" :required="true" />
      </VxValidationGroup>
      <br />
      <VxButton text="Validate" @click="() => validationGroup.validate()" />
      <br />
      <v-chip class="ma-2" color="pink" label text-color="white">
        <v-icon start icon="mdi-label"></v-icon>
        사업자번호 : {{ value }}
      </v-chip>
      <v-chip class="ma-2" color="pink" label text-color="white">
        <v-icon start icon="mdi-label"></v-icon>
        법인번호 : {{ value2 }}
      </v-chip>

      <VxMarkdownViewer :content="doc1" />
    </template>
  </v-card>
</template>
<script lang="ts" setup>
import { ref } from '#vectorx/nuxt'
import { VxMarkdownViewer } from '#vectorx/nuxt/components'
import { VxBizNoBox, VxCorpNoBox, VxValidationGroup } from '#vectorx/nuxt-vuetify/components'

// @ts-ignore
import doc1 from './md/biz-no5.md'

const value = ref(null)
const value2 = ref(null)
const validationGroup = ref<InstanceType<typeof VxValidationGroup>>()
</script>
