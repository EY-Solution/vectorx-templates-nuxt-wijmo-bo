<template>
  <VCard>
    <template #text>
      ...
      <VxButton text="팝업 오픈" @click="() => (showPopup = true)" />
      <VxMarkdownViewer :content="doc2" />
    </template>
  </VCard>
</template>
<script lang="ts" setup>
import { ref, getCurrentInstance, computed, onMounted, onBeforeUnmount, VNode } from '#vectorx/nuxt'
...
const { showPopup } = (() => {
  ...
  onMounted(() => {
    const { vNode } = $vx.utils.component.mount(RenderPopup, instance, {
      props: {
